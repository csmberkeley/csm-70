...
\binom{t}{k + 1}$ \\
We want the summations to ‚Äúmatch‚Äù. So change $t$ such that the new $t$, $t'$
goes from $k + 1$ to $n - 1$. So let $t'$Äô = $t$ - 1. Then 
$t = k + 1 \rightarrow t'Äô = k$ and $t = n \rightarrow t'Äô = n - 1$. \\
$\sum\limits_{t = k}^n \binom{t}{k} = \binom{n + 1}{k + 1} + 
